...
ui <- fluidPage(
  tagList(
    ...
    navbarPage(
            title = 
            span(h1("Kenya 2019 Census Mini App"),style = "color:black; font-weight:bolder; font-style: italic;"),
    ...
    The results were released by the","<a href='https://twitter.com/KNBStats'>Kenya National Bureau of Statistics</a>", "in February 2020, 
        ...
        tabPanel(span(style = "color:red; font-weight:bolder; font-style:italic;",  h1("Datasets")),
                 fluidRow(
                     # side bar panel
                     column(2,
                            selectInput("volume1", tags$h4(tags$b(tags$i("Select Volume"))), volumes, selected = "V1"),
        ...
        tabPanel(span(style = "color:red; font-weight:bolder; font-style:italic;",  h1("ICT Analysis")),
                 ...
                 fluidRow(
                     ...
                     ,
                     # main panel
                     column(2, DT::dataTableOutput("countytable"),br(),br(),
                            downloadButton("download8", "Download as xlsx", class = "btn-lg btn-success")),
                     column(8, plotOutput("countygraph"),br(), 
...
output$icttext <- renderText({
    ...
    paste("This dataset is a merger between V4_T2.32 and V4_T2.33")
})
  ...
  leaflet(map) %>%
    ...
    addPolygons(color = "red", weight = 1, dashArray = "3", fillColor = ~pal(variableplot) ,
                ...
                labelOptions = labelOptions(
                  ...
                  textsize = "15px",
                  direction = "auto")) %>%
    setView(38.54215, 1.010443, zoom = 5.7) %>% 
...
plotInput <- reactive({ 
      p <-  ggplot(data = countytab(), aes_string(x = "SubCounty", y = input$var))+
                ...
                labs(title = paste0("Distribution of ",input$var), subtitle = paste0("County: ", input$county),
                     x = "Sub Counties")
      if( input$var %in% grep("_Perc", input$var, value = T, ignore.case = T)){
        p <- p + ylim(c(0, max(countytab()[,2])+2 )) + coord_flip()  
      }else{
        p <- p + ylim(c(0, max(countytab()[,2])+1000 )) + coord_flip()
      }
      return(p)
})
